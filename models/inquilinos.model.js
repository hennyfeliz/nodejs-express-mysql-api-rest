...
Inquilinos.findById = (id, result) => {
    sql.query(`select * from inquilinos where id_inquilino = ${id}`, (err, res) => {
        if(err){
            console.log('error: ', err);
            result(err, null);
            return;
...
Inquilinos.updateById = (id_inquilino, inquilino, result) =>{
    sql.query (
        ...
        [inquilino.id_inquilino, inquilino.nombre, inquilino.apellido, inquilino.fecha_nacimiento, inquilino.telefono, inquilino.descripcion_preferencia, id_inquilino],
        (err, res) => {
            if(err){
                console.log('error: ', err);
                result(null, err);
...
Inquilinos.remove = (id_inquilino, result) => {
    sql.query('delete from inquilinos where id_inquilino = ?', id_inquilino, (err, res) => {
        if(err){
            console.log('error: ', err);
            result(null, err);
            return;
