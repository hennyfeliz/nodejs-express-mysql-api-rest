...
Contratos.updateById = (id_contrato, contrato, result) =>{
    sql.query (
        ...
        [contrato.id_contrato, contrato.fecha_fin_contrato, contrato.fecha_firma, contrato.fecha_inicio_alquiler, contrato.importe, contrato.id_inquilino, contrato.id_vivienda, id_contrato],
        (err, res) => {
            if(err){
                console.log('error: ', err);
                result(null, err);
...
Contratos.remove = (id_contrato, result) => {
    sql.query('delete from contrato where id_contrato = ?', id_contrato, (err, res) => {
        if(err){
            console.log('error: ', err);
            result(null, err);
            return;
